...
public class RosPublisherExample : MonoBehaviour{
    ...
    private string send_calib_val_topic="send_calib_val";
    ...
    private string pos_robot_sim_topic = "pos_robot_sim";
    private string run_sim_topic = "run_sim";
    private string robot_topic = "robot";
    private string pos_joint_robot_sim_topic = "pos_joint_robot_sim";
    ...
    PosQuatMsg posRobotSimMsg = new PosQuatMsg();
    BoolMsg startSimMsg = new BoolMsg();
    StringMsg robotMsg = new StringMsg();
    List6floatMsg posJointRobotSimMsg = new List6floatMsg();
    List3floatMsg sendCalibValMsg = new List3floatMsg();

    public static float c, opposingForce;
    public static int axis;

    public bool simAngularControl = false;
    ...
    void Start(){
        ...
        ros.RegisterPublisher<List3floatMsg>(send_calib_val_topic);
        ...
        ros.RegisterPublisher<PosQuatMsg>(pos_robot_sim_topic);
        ros.RegisterPublisher<BoolMsg>(run_sim_topic);
        ros.RegisterPublisher<StringMsg>(robot_topic);
        ros.RegisterPublisher<List6floatMsg>(pos_joint_robot_sim_topic);
    }
    ...
    public void sendCalibVal(List<float> pos){
        sendCalibValMsg.list = pos.ToArray();
        ros.Publish(send_calib_val_topic,sendCalibValMsg);
    }
    ...
    public void pubStartSim(bool startSim){
        startSimMsg.value = startSim;
        ros.Publish(run_sim_topic,startSimMsg);
    }

    public void pubRobotName(string name){
        robotMsg.value = name;
        ros.Publish(robot_topic, robotMsg);
    }

    public void pubPosRobotSim(Vector3 position, Quaternion rotation, Vector3 posIni){
        posRobotSimMsg.p_x = position.z - posIni.z;
        posRobotSimMsg.p_y = -position.x + posIni.x;
        posRobotSimMsg.p_z = position.y - posIni.y;

        posRobotSimMsg.q_w = rotation.w;
        posRobotSimMsg.q_x = -rotation.y;
        posRobotSimMsg.q_y = rotation.x;
        posRobotSimMsg.q_z = rotation.z;
    
        ros.Publish(pos_robot_sim_topic, posRobotSimMsg);
        
    }

    public void pubPosJointRobotSim(List<float> pos){
        posJointRobotSimMsg.list = pos.ToArray();
        ros.Publish(pos_joint_robot_sim_topic,posJointRobotSimMsg);
